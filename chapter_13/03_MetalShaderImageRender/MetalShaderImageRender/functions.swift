...
func createEmptyTexture(_ device: MTLDevice, width: Int, height: Int, format: MTLPixelFormat = .bgra8Unorm, length: Int = 0) -> MTLTexture {
...
func convert(image:CGImage)->CGImage?{
  guard let jsonPath = Bundle.main.path(forResource: "model/scale2.0x_model", ofType: "json") else {
    return nil
  }
  let jsonObj = try! JSONSerialization.jsonObject(with: Data(contentsOf: URL(fileURLWithPath: jsonPath)), options: []) as! [[String: AnyObject]]
  ...
  let convertedImage = createImage(sync(outTexture))
